...
import { UserModel } from '@user/models/user.shema';
...
class UserService {
  ...
  public async getUserById(userId: string): Promise<IUserDocument> {
    const users: IUserDocument[] = await UserModel.aggregate([
      // convert userID to actual mongodb object id for the aggregate method
      { $match: { _id: new mongoose.Types.ObjectId(userId) } },
      // from user schema, auth collection,
  ...
  //   return users;
  // }
  // 0 to be excluded 1 is included
  // unwind change array to object
